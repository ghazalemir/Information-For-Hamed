...
"""
"""
######This version of solid angle code stores Walpha,Wtetha,Pass inside the detector, pass inside the reactor to use them in parameter optimizatin

#Open the file the that includes particle positions (Mesh reading)

#position_particle=np.loadtxt("./Test1.txt")
pd_mesh=pd.read_csv('Mesh.csv')
position_particle=pd_mesh.values
...
def solid_angle (position_detector_face,
    ...
    wtetha=pd.DataFrame(V_W_tetha)  
    wtetha= wtetha.copy()
    wtetha.transpose().to_csv('wtetha.csv')
    
    walpha=pd.DataFrame(V_W_alpha)     
    walpha= walpha.copy()
    walpha.transpose().to_csv('walpha.csv')
    
    depthDetector=pd.DataFrame(V_V_depth_detector)     
    depthDetector= depthDetector.copy()
    depthDetector.transpose().to_csv('depthDetector.csv')
    
    depthReactor=pd.DataFrame(V_V_depth)     
    depthReactor= depthReactor.copy()
    depthReactor.transpose().to_csv('depthReactor.csv')
...
solid_angle([0.15,0,0.1],[0.17,0,0.1])
